...
def getEbaySerpCatalog(page_content, dataframe_headers):
	...
	try:
		serp_list_items = soup.find('ul',{'class':'srp-results srp-grid clearfix'}).findAll('li', recursive=False)
	except:
		serp_list_items = soup.find('ul',{'class':'srp-results srp-list clearfix'}).findAll('li', recursive=False)
